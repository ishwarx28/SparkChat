...
public class CircleImageView extends ImageView {
    ...
    public void setUserProfile(@NotNull SparkUser user) {

        Glide.with(this).load(user.getPhotoUrl() + "").placeholder(R.drawable.bg_secondary_round).addListener(new RequestListener<Drawable>() {
            @Override
            public boolean onLoadFailed(GlideException e, Object model, @NotNull Target<Drawable> target, boolean isFirstResource) {
                ...
                int originalColor = mBitmapPaint.getColor();

                mBitmapPaint.setColor(ContextCompat.getColor(getContext(), R.color.colorAccent));
                Bitmap bit = Bitmap.createBitmap(size, size, Bitmap.Config.ARGB_8888);
                Canvas c = new Canvas(bit);
                c.drawCircle(midSize, midSize, midSize, mBitmapPaint);

                mBitmapPaint.setTypeface(ResourcesCompat.getFont(getContext(), R.font.roboto_bold));
                mBitmapPaint.setTextSize(midSize);
                mBitmapPaint.setColor(Color.WHITE);
                mBitmapPaint.setTextAlign(Paint.Align.CENTER);

                c.drawText(Character.toString(firstChar), (int) midSize , midSize - (((mBitmapPaint.descent() + mBitmapPaint.ascent()) / 2)), mBitmapPaint);
                setImageBitmap(bit);

                mBitmapPaint.setColor(originalColor);
